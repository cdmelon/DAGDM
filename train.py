...
from metric import event_type_match_F1,event_seq_match_l2_F1,event_seq_match_l3_F1,event_seq_match_l4_F1,randomwalklabeled
...
def train(args,epoch,model,dataloader,optimizer):
    model.train()
    avg_loss = []
    with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
        for batch in tqdmDataLoader:
            ...
            # d_loss,pred,t,sequence_output,edge_loss = model(batch)
...
def eval(epoch,load_weight=True,test=True,test_all=False,model=None,id2event_type_dict=None, dev_data=None, test_data=None):
    ...
    with torch.no_grad():
        ...
        for input_ids,edge_mat in zip(pred,edge_mats):
            ...
            g = igraph.Graph.Adjacency(edge_mat)
            g.vs['type'] = input_ids
            G.append(g)
        ...
        if args.test_esm_while_train or test:
            ...
            dev_event_seq_match_l4_F1_score, dev_esm4_list, dev_esm4_index = event_seq_match_l4_F1(G,
                                                                                                   dev_data)  # diffusion 466 0.7569 epoch 200 289 0.2880
            logger.info('dev_avg_esm4_F1={:.4f}'.format(dev_event_seq_match_l4_F1_score))
        ...
        if test:
            ...
            event_seq_match_l4_F1_score,test_esm4_list,_ = event_seq_match_l4_F1(G,test_data)
            logger.info('test_avg_esm4_F1={:.4f}'.format(event_seq_match_l4_F1_score))
        ...
        if test_all:
            ...
            # wandb.log({("etm_etm"): test_etm_list[dev_etm_max_index]})
            ...
            logger.info('---------------------------------------')
            dev_esm2_max_index = dev_esm2_list.index(max(dev_esm2_list))
            logger.info('dev_esm2_best_etm_F1={:.4f}'.format(dev_etm_list[dev_esm2_max_index]))
            logger.info('dev_esm2_best_esm2_F1={:.4f}'.format(dev_esm2_list[dev_esm2_max_index]))
            logger.info('dev_esm2_best_esm3_F1={:.4f}'.format(dev_esm3_list[dev_esm2_max_index]))
            logger.info('test_esm2_best_etm_F1={:.4f}'.format(test_etm_list[dev_esm2_max_index]))
            logger.info('test_esm2_best_esm2_F1={:.4f}'.format(test_esm2_list[dev_esm2_max_index]))
            # wandb.log({("esm2_esm2"): test_esm2_list[dev_esm2_max_index]})
            logger.info('test_esm2_best_esm3_F1={:.4f}'.format(test_esm3_list[dev_esm2_max_index]))
            logger.info('---------------------------------------')
            ...
            # wandb.log({("esm3_esm3"): test_esm3_list[dev_esm3_max_index]})
            logger.info('---------------------------------------')
            dev_esm4_max_index = dev_esm4_list.index(max(dev_esm4_list))
            logger.info('dev_esm4_best_etm_F1={:.4f}'.format(dev_etm_list[dev_esm4_max_index]))
            logger.info('dev_esm4_best_esm2_F1={:.4f}'.format(dev_esm2_list[dev_esm4_max_index]))
            logger.info('dev_esm4_best_esm3_F1={:.4f}'.format(dev_esm3_list[dev_esm4_max_index]))
            logger.info('dev_esm4_best_esm4_F1={:.4f}'.format(dev_esm4_list[dev_esm4_max_index]))
            logger.info('test_esm4_best_etm_F1={:.4f}'.format(test_etm_list[dev_esm4_max_index]))
            logger.info('test_esm4_best_esm2_F1={:.4f}'.format(test_esm2_list[dev_esm4_max_index]))
            logger.info('test_esm4_best_esm3_F1={:.4f}'.format(test_esm3_list[dev_esm4_max_index]))
            logger.info('dev_esm4_best_esm4_F1={:.4f}'.format(test_esm4_list[dev_esm4_max_index]))
            # wandb.log({("esm4_esm4"): test_esm4_list[dev_esm4_max_index]})
            logger.info('---------------------------------------')
...
def run(seed, dataset):
    ...
    if args.eval:
        eval(epoch=0,test=True,test_all=True,model=model,id2event_type_dict=id2event_type_dict, dev_data=dev_data, test_data=test_data)
        exit()
    best_performance = 0
    best_checkpoint = None
    for epoch in range(1,args.epoch+1):
        train(args,epoch,model,dataloader,optimizer)
        if epoch % args.save_interval == 0:
            logger.info("save current model...")
            model_name = os.path.join(result_path,'model_checkpoint_{}.pth'.format(epoch))
            torch.save(model.state_dict(), model_name)
        if epoch%args.test_interval == 0:
            performance = eval(epoch,load_weight=False,test=False,test_all=False,model=model,id2event_type_dict=id2event_type_dict, dev_data=dev_data, test_data=test_data)
    ...
    model.load_state_dict(ckpt)
    print("model load weight done.")
    eval(epoch=0,load_weight=False,test_all=True,model=model,id2event_type_dict=id2event_type_dict, dev_data=dev_data, test_data=test_data)


for dataset in ['suicide_ied', 'wiki_mass_car_bombings']:
