...
def event_seq_match_l4_F1(G,G_true):
    G_F1_list = []
    with Pool(15) as p:
        for idx,_F1 in enumerate(tqdm(p.imap(event_seq_match_l4_F1_single,zip(G,[G_true for _ in range(len(G))])),total=len(G))):
            G_F1_list.append(_F1)
    index = G_F1_list.index(max(G_F1_list))
    return sum(G_F1_list)/len(G_F1_list),G_F1_list,index


def event_seq_match_l4_F1_single(data):
    g,G_true = data
    sum_F1 = 0
    g_event_seq_3 = []
    adjlist = g.get_adjlist()
    for i in range(len(adjlist)):
        for j in adjlist[i]:
            for k in adjlist[j]:
                for l in adjlist[k]:
                    g_event_seq_3.append(str(g.vs['type'][i])+'->'+str(g.vs['type'][j])+'->'+str(g.vs['type'][k])+'->'+str(g.vs['type'][l]))
    g_event_seq_3 = set(g_event_seq_3)
    for _,g_true in enumerate(G_true):
        try:
            TP = 0
            ...
            for i in range(len(adjlist)):
                for j in adjlist[i]:
                    for k in adjlist[j]:
                        for l in adjlist[k]:
                            g_true_event_seq_3.append(str(g.vs['type'][i])+'->'+str(g.vs['type'][j])+'->'+str(g.vs['type'][k])+'->'+str(g.vs['type'][l]))
            g_true_event_seq_3 = set(g_true_event_seq_3)
            curr_list_true = g_true_event_seq_3
            curr_list_pred = g_event_seq_3
            intersection = list((Counter(curr_list_true) & Counter(curr_list_pred)).elements())
